...
public class HazardMechanics : MonoBehaviour
{
    ...
    protected void RunHazard(float sliderProgress, Transform cameraFocalPoint, int index)
    {
        ...
        else //If camera is in position, minigame will start
        {
            ...
            {
                hazardManager.FinishHazard(Score.GetScore(hazardManager.hazardName).timeGained, Score.GetScore(hazardManager.hazardName).score, true, cameraFocalPoint, hazardTransform, index);
            ...
            {
                hazardManager.FinishHazard(Score.GetScore(hazardManager.hazardName).timeLost, 0, false, cameraFocalPoint, hazardTransform, index);
                GetComponent<Collider>().enabled = true;
            }
